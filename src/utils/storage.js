...
export function isAutoGeneratedNote(note) {
  ...
  return (
    lower.startsWith('auto-marked') ||
    lower.startsWith('auto marked') ||
    lower.startsWith('automarked') ||
    lower.includes('auto-mark') ||
    lower.includes('auto mark') ||
    lower.includes('automark') ||
    lower.includes('otomatik işaretlendi') ||
    lower.includes('otomatik olarak') ||
    lower.includes('bölge ziyareti') ||
    lower.includes('şehir ziyareti') ||
    lower.includes('il ziyareti') ||
    lower.includes('alt bölge') ||
    lower.includes('ziyareti)') ||
    lower.includes('ziyareti ile') ||
    lower.includes('ziyaretiyle')
  );
}
    ...
    if (rawTurkey) {
      ...
      if (p && typeof p === 'object' && !Array.isArray(p)) {
        ...
        for (const k in turkeyVisits) {
          if (turkeyVisits[k]) {
            if (isAutoGeneratedNote(turkeyVisits[k].notes)) {
              turkeyVisits[k].notes = '';
              changed = true;
            }
            if (turkeyVisits[k].journal && isAutoGeneratedNote(turkeyVisits[k].journal.text)) {
              turkeyVisits[k].journal.text = '';
              changed = true;
            }
            // Purge legacy auto-default rating of 5 if user never rated or wrote a note
            if (turkeyVisits[k].rating === 5 && !cleanNote(turkeyVisits[k].notes) && !turkeyVisits[k].hasCustomRating) {
              turkeyVisits[k].rating = 0;
              changed = true;
            }
          }
        }
    ...
    if (rawWorld) {
      ...
      if (p && typeof p === 'object' && !Array.isArray(p)) {
        ...
        for (const k in worldVisits) {
          if (worldVisits[k]) {
            if (isAutoGeneratedNote(worldVisits[k].notes)) {
              worldVisits[k].notes = '';
              changed = true;
            }
            if (worldVisits[k].journal && isAutoGeneratedNote(worldVisits[k].journal.text)) {
              worldVisits[k].journal.text = '';
              changed = true;
            }
            // Purge legacy auto-default rating of 5 if user never rated or wrote a note
            if (worldVisits[k].rating === 5 && !cleanNote(worldVisits[k].notes) && !worldVisits[k].hasCustomRating) {
              worldVisits[k].rating = 0;
              changed = true;
            }
          }
        }
...
export function saveTurkeyVisit(provinceId, status, details = {}) {
  ...
  if (status === 'unvisited' || !status) {
    ...
    turkeyVisits[provinceId] = {
      ...
      hasCustomRating: details.hasCustomRating !== undefined 
        ? Boolean(details.hasCustomRating) 
        : (details.rating !== undefined ? (Number(details.rating) > 0) : Boolean(existing.hasCustomRating)),
