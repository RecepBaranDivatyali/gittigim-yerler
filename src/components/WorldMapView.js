...
function getFlagHtml(code) {
  if (!code || code.toUpperCase() === 'IL') return '';
...
function openStatusPopup(latlng, id, title, type, countryCode, feature = null) {
  ...
  if (type === 'province') {
    ...
    currentNotes = cleanNote(pData.notes);
  ...
  if (currentJournal && currentJournal.text) {
    currentJournal.text = cleanNote(currentJournal.text);
  }
  ...
  journalTextInput?.addEventListener('blur', () => {
    ...
    const existingText = cleanNote(currentJournal?.text || currentNotes || '').trim();
    if (jText !== existingText) {
      currentJournal = {
        ...(currentJournal || {}),
        text: jText,
        mood: selectedMood,
...
function countryStyle(c) {
  ...
  const fillOpacity = status === 'unvisited' ? 0.95 : cfg.fillOpacity;
